...
export default function BeliefOption(belief: Readonly<BeliefOptionProps>) {
    const queryClient = useQueryClient();


    const deleteBelief = async () => {
        try{
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/beliefs/${belief.id}`, {
                method: 'DELETE',
            });

            console.log(`Response: ${JSON.stringify(response)}`);

            queryClient.invalidateQueries('beliefs');
        }catch(e){
            console.log(`Error: ${e}`);
        }
    };
    ...
    <div className={`${belief.styles}`}>
        ...
        <div className='w-full'>
            ...
            <div className='w-full flex items-center justify-between'>
                <div>
                    ...
                    {/* Belief Tratis*/}
                    { (belief.traits && belief.traits.length > 0) && 
                    <p className='italic font-light'>
                        Traits:
                    </p>}
                    {belief.traits.map((trait: any) => {
                        return (
        ...
        <div>
            <h5 className="invisible group-hover:visible mx-1 rounded-lg px-3 py-1 bg-black border hover:bg-purple-300/10 border-yellow-900/50
                                 active:translate-y-1 text-xl cursor-pointer text-yellow-200/70 " onClick={deleteBelief}><IoTrashOutline/></h5>
        </div>
        </div>
    </div>
    )
