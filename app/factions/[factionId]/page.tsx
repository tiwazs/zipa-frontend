...
import DetailedFactionChart from './DetailedFactionChart';
...
const getFaction = async (factionId: string) => {
    ...
    try{
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/factions/${factionId}`, {
            method: 'GET',
        });
...
export default function FactionPage({ params: { factionId } }: FactionPageProps) {
    const query = useQuery([`faction`, factionId], () => getFaction(factionId) );
    ...
    return (
        <>
            <main className="items-center justify-between p-24">
                ...
                {/*Faction Data*/}
                <DetailedFactionChart faction={query.data} />
            </main>           
        </>
    )
}
