...
interface CreateBeliefFormOptions {
    name: string;
    description: string;
    identity: string;
    aspects: string;
...
interface NewBeliefDialogProps {
    styles?: string;
}

export default function NewBeliefDialog({styles}: Readonly<NewBeliefDialogProps>) {
    let [isOpen, setIsOpen] = useState(false);
    const { register, handleSubmit } = useForm<CreateBeliefFormOptions>();
    
    const queryClient = useQueryClient();
    
    const onSubmit: SubmitHandler<CreateBeliefFormOptions> = async data => {
        console.log(data);

        // Not setting the content type. aparently the browser will do that for us, including the boundary
        try{
          const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/beliefs/`, {
              method: "POST",
              headers: {
