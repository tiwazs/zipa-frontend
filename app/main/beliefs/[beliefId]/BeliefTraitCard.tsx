...
interface BeliefTraitCardProps {
    beliefId: string;
    belieftrait: any;
    editable: boolean;
}

export default function BeliefTraitCard({ beliefId, belieftrait, editable }: Readonly<BeliefTraitCardProps>) {
    const trait = belieftrait.trait;
    const queryClient = useQueryClient();

    const deleteTrait = async () => {
        try{
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/beliefs/remove_trait/${beliefId}?trait_id=${trait.id}`, {
                method: 'PUT',
            });
    ...
    return (
        <div className='group my-2 px-4 py-2 font-light dark:border-2 rounded-md dark:border-yellow-900/50 flex items-center justify-between '>
            ...
            <div className='flex items-center space-x-2'>
                <img src={`${process.env.NEXT_PUBLIC_API_URL}/static/traits/${belieftrait.trait.id}.jpg`} alt="" className='w-12 h-12 rounded-md border-2 border-gray-500/60 my-2' />
                <p>
                    <Link href={`/main/traits/${belieftrait.trait.id}`}><span className='text-yellow-400 font-normal'>{trait.name}</span></Link>
                </p>
            </div>
            <p>
