...
import DetailedBeliefChart from './DetailedBeliefChart';

type BeliefPageProps = {
    params: {
        beliefId: string;
    }
}

const getBelief = async (beliefId: string) => {

    try{
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/beliefs/${beliefId}`, {
            method: 'GET',
        });
...
export default function BeliefPage({ params: { beliefId } }: Readonly<BeliefPageProps>) {
    const query = useQuery([`belief`, beliefId], () => getBelief(beliefId) );
    ...
    return (
        <main className="items-center justify-between p-24">
            ...
            {/*Belief Data*/}
            <DetailedBeliefChart belief={query.data} />
        </main>
    )
}
