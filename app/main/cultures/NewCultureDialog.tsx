...
interface CreateCultureFormOptions {
    name: string;
    description: string;
    identity: string;
    aspects: string;
...
export default function NewCultureDialog({styles}: Readonly<NewCultureDialogProps>) {
    let [isOpen, setIsOpen] = useState(false);
    const { register, handleSubmit } = useForm<CreateCultureFormOptions>();
    
    const queryClient = useQueryClient();
    
    const onSubmit: SubmitHandler<CreateCultureFormOptions> = async data => {
        console.log(data);

        // Not setting the content type. aparently the browser will do that for us, including the boundary
        try{
          const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/cultures/`, {
              method: "POST",
              headers: {
