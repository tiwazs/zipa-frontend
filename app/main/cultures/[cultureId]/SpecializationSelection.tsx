...
export default function SpecializationSelection({cultureId}: Readonly<SkillSelectionProps>) {
    ...
    const onAddSkill = async () => {
      ...
      try{
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/cultures/add_unit/${cultureId}?unit_id=${selected.id}`, {
            method: "PUT",
            headers: {
        ...
        <Listbox value={selected} onChange={setSelected}>
          <div className="relative mt-1">
            <Listbox.Button className="relative w-full cursor-default rounded-lg bg-black hover:bg-purple-300/10 text-yellow-200/70 py-2 pl-3 pr-10 text-left shadow-md focus:outline-none
              ...
              <span className="block truncate">{selected ? selected.name : "Select a Specialization"}</span>
