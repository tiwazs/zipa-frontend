...
import DetailedCultureChart from './DetailedCultureChart';
...
const getCulture = async (cultureId: string) => {

    try{
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/cultures/${cultureId}`, {
            method: 'GET',
        });
...
export default function CulturePage({ params: { cultureId } }: Readonly<CulturePageProps>) {
    const query = useQuery([`culture`, cultureId], () => getCulture(cultureId) );
    ...
    return (
        <main className="items-center justify-between p-24">
            ...
            {/*Culture Data*/}
            <DetailedCultureChart culture={query.data} />
        </main>
    )
}
