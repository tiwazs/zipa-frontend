...
interface CreateRaceGroupFormOptions {
    name: string;
    description: string;
    identity: string;
    aspects: string;
...
interface NewRaceGroupDialogProps {
    styles?: string;
}

export default function NewRaceGroupDialog({styles}: Readonly<NewRaceGroupDialogProps>) {
    let [isOpen, setIsOpen] = useState(false);
    const { register, handleSubmit } = useForm<CreateRaceGroupFormOptions>();
    
    const queryClient = useQueryClient();
    
    const onSubmit: SubmitHandler<CreateRaceGroupFormOptions> = async data => {
        console.log(data);
                ...
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl p-6 text-left shadow-xl transition-all 
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-400">
                      Create a New Race Group. this is just a definition that groups related races that have similar units, beliefs, cultures etc.
                    </p>
