...
import DetailedSkillChart from './DetailedSkillChart';
...
const getSkill = async (skillId: string) => {
    ...
    try{
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/skills/${skillId}`, {
            method: 'GET',
        });
...
export default function SkillPage({ params: { skillId } }: SkillPageProps) {
    const query = useQuery([`skill`, skillId], () => getSkill(skillId) );
        ...
        <>
            <main className="items-center justify-between p-24">
                <div className='flex items-center space-x-3'>
                    <img src={`${process.env.NEXT_PUBLIC_API_URL}/static/skills/${query.data.id}.jpg`} alt="" className='w-14 h-14 rounded-md border-2 border-gray-500/60 my-2' />
                ...
                {/*Skill Data*/}
                <DetailedSkillChart skill={query.data} />
            </main>           
        </>
    )
}
