...
import DetailedSpecializationChart from './DetailedSpecializationChart';
...
const getSpecialization = async (specializationId: string) => {

    try{
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/specializations/${specializationId}`, {
            method: 'GET',
        });
...
export default function SpecializationPage({ params: { specializationId } }: SpecializationPageProps) {
    const query = useQuery([`specialization`, specializationId], () => getSpecialization(specializationId) );
    ...
    return (
        <>
            <main className="items-center justify-between p-24">
                ...
                {/*Specialization Data*/}
                <DetailedSpecializationChart specialization={query.data} />
            </main>           
        </>
    )
}
