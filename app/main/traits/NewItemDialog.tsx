...
interface CreateTraitFormOptions {
    name: string;
    description: string;
    effects?: string[];
}

interface NewTraitDialogProps {
    styles?: string;
}

export default function NewTraitDialog({styles}: NewTraitDialogProps) {
    let [isOpen, setIsOpen] = useState(false);
    const { register, handleSubmit, watch, formState: { errors } } = useForm<CreateTraitFormOptions>();
    
    const queryClient = useQueryClient();
    
    const onSubmit: SubmitHandler<CreateTraitFormOptions> = async data => {
        console.log(data);
                ...
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl p-6 text-left shadow-xl transition-all 
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-400">
                      Create a new trait. traits are unique characteristics or passive effects of characters, and last indefinitely. traits can be positive or negative,
                      these can be inflicting terror on enemies, or being more resiliant to damage, or even being able to fly, or many other things.
                    </p>
