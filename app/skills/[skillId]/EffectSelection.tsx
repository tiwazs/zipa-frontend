...
export default function EffectSelection({skillId}: EffectSelectionProps) {
    ...
    const onAddEffect = async () => {

      // Not setting the content type. aparently the browser will do that for us, including the boundary
      try{
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/skills/add_effect/${skillId}?effect_id=${selected.id}&duration=${duration}`, {
            method: "PUT",
        });
        ...
        <Listbox value={selected} onChange={setSelected}>
          <div className="relative mt-1">
            <Listbox.Button className="relative w-full cursor-default rounded-lg bg-black hover:bg-purple-300/10 text-yellow-200/70 py-2 pl-3 pr-10 text-left shadow-md focus:outline-none
              ...
              <span className="block truncate">{selected ? selected.name : "Select an Effect"}</span>
