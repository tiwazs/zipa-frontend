...
export default function ItemSelection({specializationId}: ItemSelectionProps) {
    ...
    const onAddItem = async () => {
      ...
      try{
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/specializations/add_item/${specializationId}`, {
            method: "PUT",
            headers: {
        ...
        <Listbox value={selected} onChange={setSelected}>
          <div className="relative mt-1">
            <Listbox.Button className="relative w-full cursor-default rounded-lg bg-black hover:bg-purple-300/10 text-yellow-200/70 py-2 pl-3 pr-10 text-left shadow-md focus:outline-none
              ...
              <span className="block truncate">{selected ? selected.name : "Select an Item"}</span>
      ...
      <div className='flex flex-col w-full'>
        <div className='flex items-center'>
            ...
            <input className='w-full my-2 rounded-lg py-3 text-gray-400 text-md bg-[#2b2532] bg-opacity-10 focus:bg-opacity-30 focus:outline-none border dark:border-yellow-900/50'
                     value={quantity} type="text" id="quantity" onChange={(e)=>{setQuantity(e.target.value)}}/>
        </div>
      </div>
      <button
          type="button"
